...
/// A single sentence row in side-by-side mode
private struct SideBySideSentenceRow: Identifiable {
    let id: String
...
/// A statement/answer block pair in side-by-side mode
private struct SideBySideBlockPair: Identifiable {
    let id: Int
    var statement: [SideBySideSentenceRow] = []
    var answer: [SideBySideSentenceRow] = []
}

struct SideBySideMainPanel: View {
    var docID: String = "Roos.record.2007"   // 👈 can pass in later if needed

    @State private var sections: [SideBySideBlockPair] = []
    ...
    private func load() {
        ...
        // Use the app's working directory DB instead of hardcoded path
        ...
        var map: [Int: SideBySideBlockPair] = [:]
        var idx = 0
        var count = 0
        ...
        while sqlite3_step(stmt) == SQLITE_ROW {
            ...
            var pair = map[bn] ?? SideBySideBlockPair(id: bn)
            if bt == "statement" {
...
#Preview {
    NavigationView {
        SideBySideMainPanel(docID: "Roos.record.2007")
    }
}
